...
```bash
pipx install tap-nubank
```

Install from GitHub:

```bash
pipx install git+https://github.com/ORG_NAME/tap-nubank.git@main
```
...
```
tap-nubank --about --format=markdown
```
-->
...
You can also test the `tap-nubank` CLI interface directly using `poetry run`:

```bash
poetry run tap-nubank --help
```
